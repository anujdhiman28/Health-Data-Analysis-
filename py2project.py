...
class HealthDataAnalyzer:
    ...
    def visualize_data(self):
        if self.data is not None:
            ...
            column_name = simpledialog.askstring("Input", f"Available columns: {available_columns}\nEnter the column name to visualize:")
            
            if column_name in available_columns:
                plt.figure(figsize=(8, 6))
                plt.hist(self.data[column_name], bins=10, edgecolor='black')
                ...
                messagebox.showwarning("Warning", f"'{column_name}' column not found in data.")
        else:
            messagebox.showwarning("Warning", "No data uploaded!")
